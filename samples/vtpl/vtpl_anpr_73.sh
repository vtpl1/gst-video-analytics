...
gvadetect model-instance-id=inf0 model=${DETECT_MODEL_PATH} device=${INFERENCE_DEVICE} pre-process-backend=${PRE_PROC} model-proc=${DETECT_POST_PROC_PATH} ! queue ! \
gvadetect model-instance-id=inf1 model=${OCR_MODEL_PATH} device=${INFERENCE_DEVICE} pre-process-backend=${PRE_PROC} model-proc=${OCR_POST_PROC_PATH} is-full-frame=false object-class="LP" ! queue !
